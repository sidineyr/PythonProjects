...
class TonePlayer {
    ...
    fun play(frequencies: List<Double>, volume: Double = 0.45, noteMs: Int = 420, onDone: () -> Unit = {}) {
        if (playing) return
        playing = true
        thread(name = "somente-tone") {
            try {
                frequencies.forEach { playOne(it, volume, noteMs) }
            } finally {
                playing = false
                onDone()
            }
    ...
    private fun playOne(frequency: Double, volume: Double, durationMs: Int) {
        ...
        val samples = ShortArray(count) { index ->
            ...
            (sin(2.0 * PI * index * frequency / sampleRate) * Short.MAX_VALUE * volume * fade).toInt().toShort()
        }
