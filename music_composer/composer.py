...
def compose_melody(
    ...
    """Generate a melody constrained to a musical scale and bar length."""
    ...
    while cursor < total_beats:
        ...
        if rng.random() < randomness:
            ...
            note = min(notes, key=lambda candidate: abs(candidate - previous))
            index = notes.index(note)
            step = rng.choice((-1, 0, 1))
            note = notes[max(0, min(len(notes) - 1, index + step))]
